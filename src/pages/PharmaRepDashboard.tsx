...
const PharmaRepDashboard = () => {
  ...
  const filteredReviews = reviews.filter(review => review.medicineName.toLowerCase().includes(searchTerm.toLowerCase()) || review.condition.toLowerCase().includes(searchTerm.toLowerCase()));
          ...
          <CardContent className="pt-0">
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={medicineEffectivenessData} margin={{
                top: 20,
                right: 30,
                left: 20,
                bottom: 60
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={sideEffectsData} cx="50%" cy="50%" outerRadius={120} fill="#8884d8" dataKey="value" label={({
                  name,
                  percent
                }) => `${name} ${(percent * 100).toFixed(0)}%`}>
                    {sideEffectsData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                  </Pie>
                  <Tooltip />
                  <Legend />
                </PieChart>
        ...
        <CardContent className="pt-0">
          <div className="h-80">
            ...
            count: {
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={conditionPrevalenceData} margin={{
                top: 20,
                right: 30,
                left: 20,
                bottom: 5
      ...
      <Card>
          ...
          <div className="mt-4 sm:mt-0 w-full sm:w-auto">
            <div className="flex gap-2">
              <div className="relative w-full sm:w-64">
                <Input type="text" placeholder="Search by medicine or condition..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pr-8" />
              </div>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {filteredReviews.map(review => <TableRow key={review.id}>
          ...
          <div className="mt-6 space-y-4">
            ...
            {filteredReviews.map(review => <Card key={review.id} className="bg-slate-50">
                <CardHeader className="pb-2">
